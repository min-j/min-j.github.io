...
export const experience: ExperienceItem[] = [
  {
    ...
    bullets: [
      ...
      'Designed and shipped a domain migration tool that consolidated a complex, multi-step manual process into a single automated operation with dry-run pre-checks, phase-selectable execution, and per-phase logging — transitioning 2,000+ customer records to a new domain architecture.',
      'Built a CSV ingestion framework across three data pipelines (users, devices, sites) with dry-run previews, error tracking, and exportable logs, directly unblocking enterprise-scale customer onboarding.',
      'Proposed and led a codebase-wide dependency modernization effort, resolving security vulnerabilities and completing a full Vuex → Pinia migration across 19 store modules.',
      'Led full-stack API naming standardization across 250+ endpoints and 15+ microservices, resolving convention inconsistencies to improve maintainability.',
      'Delivered occupancy sensor views in a React/Next.js device management portal, surfacing multi-zone telemetry data ahead of a coordinated firmware release.',
    ],
  ...
  },
  {
    ...
    bullets: [
      'Replaced per-device API status calls with a batched request strategy, making status retrieval near-instantaneous and resolving a data accuracy issue in a single architectural change.',
      'Identified and resolved a sequential device command execution bottleneck, replacing it with parallel processing and achieving a 16× performance improvement across multi-device operations.',
      'Built user management tooling covering sign-in history, temporary password provisioning, and user-domain assignment search.',
      'Overhauled domain and device search — adding MAC address lookup, fixing substring matching, and handling special characters — eliminating a class of recurring lookup failures.',
      'Implemented MFA enforcement, configurable session timeouts, and activity tracking using AWS Cognito in direct response to security concerns flagged by the principal engineer.',
    ],
